...
export function ImgNameTagDiv({
                                  name, logo, recipe, recipeName, colour, tagWidth,
                                  imgLeftMob = "1.7rem",
                                  imgLeftDesc = "2.0rem",
                                  pLeftMob = "5.8rem",
    ...
    return (
        <div className={cn("relative w-full flex flex-row", className)} {...props}>
            ...
            <div style={{color: colour, background: bgColour, width: tagWidth}}
                 className="absolute right-[1.8rem] md:right-[2.0rem] h-[2.6rem] text-center text-[1.6rem] font-nunito border rounded-[1rem] font-normal">{recipe[recipeName]} {addText}</div>
        </div>
    );
}