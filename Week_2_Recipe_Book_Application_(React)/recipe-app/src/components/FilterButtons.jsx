...
export function FilterButtons({filter, onFilterChange, className = '', ...props}) {
    ...
    return (
        <div
            className={cn("relative grid grid-cols-3 lg:flex items-center self-center md:self-end gap-[1.0rem]", className)}
            {...props}
        >
            {Object.values(Difficulty).map((level) => (
                <button
                    ...
                    className={cn(
                        baseStyles,
                        filter === level && "text-[var(--blue)] bg-[var(--light-blue)] active",
