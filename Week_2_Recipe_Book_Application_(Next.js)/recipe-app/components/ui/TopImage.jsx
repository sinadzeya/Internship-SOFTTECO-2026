...
export function TopImage({ title, className = "", ...props }) {
	const baseStyles =
		"w-full h-[16.0rem] md:h-[33.6rem] flex items-center justify-center bg-cover bg-center border border-black";

	return (
		<div
			className={cn(baseStyles, className)}
			style={{
				backgroundImage: `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.3)), url(${mainImage.src})`,
			}}
			{...props}
		>
